...
// Security: Prevent directory traversal
	function isSafePath($path, $baseDir)
	{
		...
		// If file doesn't exist yet (for saving), check dirname
		if (!$realPath) {
	...
	switch ($action) {
		...
		case 'list_scenes':
			...
			if (!is_dir($fullPath) || !isSafePath($fullPath, $baseSceneDir)) {
				// Default to root scenes if invalid
		...
		case 'load_scene':
			$filename = $_POST['filename'] ?? ''; // Can include relative path e.g. "level1/start.json"
